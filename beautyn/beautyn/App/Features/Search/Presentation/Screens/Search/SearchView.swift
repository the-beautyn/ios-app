...
struct SearchView: BaseViewProtocol {
    ...
    var contentView: some View {
        ZStack(alignment: .top) {
            // Map + its controls together, so the results sheet slides over both.
            ZStack(alignment: .top) {
                mapLayer
            ...
            SearchHeaderView(
                ...
                onLocationTap: { viewModel.didTapLocation() }
            )
        }
    }

    private var userLocationButton: some View {
        // "scope" (crosshair) — deliberately distinct from the header's
        // location-arrow button.
        GlassCircleButton(icon: Image(systemName: "scope")) {
            viewModel.didTapMapLocationButton()
        }
        .frame(maxWidth: .infinity, alignment: .trailing)
        .padding(.trailing, CGFloat.Spacing.md)
        .padding(.top, SearchHeaderView.contentHeight + CGFloat.Spacing.sm)
        .alert(
    ...
    private func clusterBubble(_ cluster: SearchViewModel.MapCluster) -> some View {
        Button {
